...
namespace BEZNgCore.Tests.Authorization.Users;
...
public class UserAppService_Link_Tests : UserAppServiceTestBase
{
    ...
    public async Task Should_Link_User_To_Different_Tenant_User()
    {
        ...
        //Assert
        var defaultTenantAdmin = await UsingDbContextAsync(async context => await context.Users.FirstOrDefaultAsync(u => u.TenantId == AbpSession.TenantId && u.Id == AbpSession.UserId));
        var testUser = await UsingDbContextAsync(testTenantId, async context => await context.Users.FirstOrDefaultAsync(u => u.UserName == "test"));

        var areUsersLinked = await _userLinkManager.AreUsersLinked(defaultTenantAdmin.ToUserIdentifier(), testUser.ToUserIdentifier());
        areUsersLinked.ShouldBe(true);
    }
    ...
    public async Task Should_Link_User_To_Already_Linked_User()
    {
        ...
        //Assert
        var defaultTenantAdmin = await UsingDbContextAsync(async context => await context.Users.FirstOrDefaultAsync(u => u.TenantId == AbpSession.TenantId && u.UserName == AbpUserBase.AdminUserName));
        var jnash = await UsingDbContextAsync(async context => await context.Users.FirstOrDefaultAsync(u => u.UserName == "jnash"));

        var testTenantUser = await UsingDbContextAsync(testTenantId, async context => await context.Users.FirstOrDefaultAsync(u => u.UserName == "test"));

        var areUsersLinked = await _userLinkManager.AreUsersLinked(defaultTenantAdmin.ToUserIdentifier(), testTenantUser.ToUserIdentifier());
        areUsersLinked.ShouldBe(true);
        
        areUsersLinked = await _userLinkManager.AreUsersLinked(jnash.ToUserIdentifier(), testTenantUser.ToUserIdentifier());
        areUsersLinked.ShouldBe(true);
    }
    ...
    private async Task<int> CreateTestTenantAndTestUser()
    {
        ...
        {
            ConnectionString = SimpleStringCipher.Instance.Encrypt("Server=localhost; Database=BEZNgCoreTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;")
        };
    ...
    private async Task LinkUserAndTestAsync(string tenancyName)
    {
        ...
        //Assert
        var linkedUser = await UsingDbContextAsync(async context => await context.Users.FirstOrDefaultAsync(u => u.UserName == "jnash"));
        var linkedUserAccount = await _userLinkManager.GetUserAccountAsync(linkedUser.ToUserIdentifier());

        var currentUser = await UsingDbContextAsync(async context => await context.Users.FirstOrDefaultAsync(u => u.Id == AbpSession.UserId));
        var currentUserAccount = await _userLinkManager.GetUserAccountAsync(currentUser.ToUserIdentifier());

       var areUsersLinked =  await _userLinkManager.AreUsersLinked(currentUser.ToUserIdentifier(), linkedUser.ToUserIdentifier());
       areUsersLinked.ShouldBe(true)
