...
namespace BEZNgCore.Authentication.TwoFactor.Google;

public class GoogleAuthenticatorProvider : BEZNgCoreServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
{
    ...
    public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
    {
        ...
        var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("BEZNgCore", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
