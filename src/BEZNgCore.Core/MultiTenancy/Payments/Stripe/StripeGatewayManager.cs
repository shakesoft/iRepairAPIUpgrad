...
public class StripeGatewayManager : BEZNgCoreServiceBase,
    ...
    ITransientDependency
{
    ...
    public async Task HandleInvoicePaymentSucceededAsync(Invoice invoice)
    {
        ...
        var items = new List<RecurringPaymentLineItem>();
        foreach (var i in invoice.Lines.Data)
        {
            var item = new RecurringPaymentLineItem
            {
                Product = i.Pricing?.PriceDetails?.Product,
                PlanId = i.Pricing?.PriceDetails?.Price,
                PlanType = await GetPlanTypeByIdAsync(i.Pricing?.PriceDetails?.Price),
        ...
        await EventBus.TriggerAsync(
            new RecurringPaymentSucceedEventData(tenantId, invoice.Id, invoice.AmountPaid)
            {
                Items = items
            }
        );
        ...
        string productId)
    {
        ...
        var plan = await planService.CreateAsync(new PlanCreateOptions
        {
            ...
            Interval = interval,
            Product = productId,
            Currency = BEZNgCoreConsts.Currency,
    ...
    public async Task<StripeIdResponse> GetOrCreatePlanForPayment(long paymentId)
    {
        ...
        var product = await GetOrCreateProductAsync(BEZNgCoreConsts.ProductName);
    ...
    private async Task<string> GetPlanTypeByIdAsync(string priceId)
    {
        var planService = new PlanService();
        var plan = await planService.GetAsync(priceId);
        return plan.Metadata.GetValueOrDefault(PaymentConsts.PlanType);
    }
}
