...
public class BEZNgCoreAsyncJwtBearerHandler : AuthenticationHandler<AsyncJwtBearerOptions>
{
    ...
    /// </summary>
    /// <inheritdoc />
    public BEZNgCoreAsyncJwtBearerHandler(IOptionsMonitor<AsyncJwtBearerOptions> options,
        ...
        try
        {
            ...
            foreach (var validator in Options.AsyncSecurityTokenValidators)
            {
                if (validator.CanReadToken(token))
                {
                    ...
                    try
                    {
                        (principal, asyncValidatedToken) = await validator.ValidateToken(token, validationParameters);
                        return await OnTokenValidationSucceed(principal, asyncValidatedToken, token);
                    }
                    catch (Exception ex)
                    {
