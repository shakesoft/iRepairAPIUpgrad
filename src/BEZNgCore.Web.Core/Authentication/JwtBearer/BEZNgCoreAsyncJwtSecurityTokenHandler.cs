...
public class BEZNgCoreAsyncJwtSecurityTokenHandler : IAsyncSecurityTokenValidator
{
    private readonly JwtSecurityTokenHandler _tokenHandler;

    public BEZNgCoreAsyncJwtSecurityTokenHandler()
    {
        ...
        TokenValidationParameters validationParameters)
    {
        ...
        return await ValidateTokenInternal(principal, validatedToken, true);
    }
    ...
    public async Task<(ClaimsPrincipal, SecurityToken)> ValidateTwoFactorRememberClientToken(string securityToken,
        TokenValidationParameters validationParameters)
    {
        var principal = _tokenHandler.ValidateToken(securityToken, validationParameters, out var validatedToken);

        if (!HasTokenType(principal, TokenType.TwoFactorRememberClientToken))
        {
            throw new SecurityTokenException("invalid token type");
        }

        return await ValidateTokenInternal(principal, validatedToken, false);
    }

    private async Task<(ClaimsPrincipal, SecurityToken)> ValidateTokenInternal(
        ClaimsPrincipal principal,
        SecurityToken validatedToken,
        bool validateSecurityStamp)
    {
        var cacheManager = IocManager.Instance.Resolve<ICacheManager>();
        if (validateSecurityStamp)
        {
