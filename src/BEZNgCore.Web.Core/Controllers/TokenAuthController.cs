    ...
    public async Task<AuthenticateResultModel> AuthenticatePINMobileIR([FromBody] AuthenticateModel model)
    {
        ...
        //GetTenancyNameOrNull()
            ...
            var accessToken = CreateAccessToken(await CreateJwtClaims(loginResult.Identity, loginResult.User,
                refreshTokenKey: refreshToken.key));
            bool IsAdmin = false;
            bool ChangeCleanStatus = false;
            if (loginResult.User.StaffKey != Guid.Empty)
            {
                using (UnitOfWorkManager.Current.SetTenantId(loginResult.Tenant?.Id))
                {
                    var getStaff = _staffRepository.GetAll()
                    .Where(x => x.Id == loginResult.User.StaffKey)
                    .Select(x => x).FirstOrDefault();
                    if (getStaff.Sec_Supervisor != null)
                    {
                        if (getStaff.Sec_Supervisor == 10)
                            IsAdmin = true;
                    }
                    ChangeCleanStatus = getStaff.ChangeCleanStatus != null ? ChangeCleanStatus : false;
                }
            ...
            return new AuthenticateResultModel
            {
                ...
                IsAdmin = IsAdmin,
                ChangeCleanStatus = ChangeCleanStatus
        ...
        else
        {
            ...
            .Where(x => x.Id == loginResult.User.StaffKey)
            .Select(x => new { x.Sec_Supervisor, x.Sec_TechSupervisor, x.Sec_BlockRoom, x.Sec_IPSetUp, x.Sec_IPViewLog, x.Sec_IPAssignTasks, x.Sec_IPBlockRoom, x.ChangeCleanStatus })
            .FirstOrDefault();
                    ...
                    if (accessstaff != null)
                    {
                        ...
                        if (accessstaff.Sec_IPBlockRoom.HasValue && accessstaff.Sec_IPBlockRoom.Value == 10)
                            SecIPBlockRoom = true;
                        if (accessstaff.ChangeCleanStatus.HasValue)
                            ChangeCleanStatus = Convert.ToBoolean(accessstaff.ChangeCleanStatus.Value);
                    }
