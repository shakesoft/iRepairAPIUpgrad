...
namespace BEZNgCore.Web.Controllers
{
    ...
    public class TokenAuthIciController : BEZNgCoreControllerBase
    {
        private const string UserIdentifierClaimType = "http://aspnetzero.com/claims/useridentifier";
        private readonly LogInManagerIcheckIn _logInManagerIcheckIn;
        ...
        public TokenAuthIciController(
            LogInManagerIcheckIn logInManagerIcheckIn,
        ...
        {
          _logInManagerIcheckIn = logInManagerIcheckIn;
        ...
        #region icheckin
        [HttpPost]
        public async Task<AuthenticateResultIciModel> AuthenticateIcheckIn([FromBody] AuthenticateModel model)
        {
            ...
            var loginResult = await GetLoginResultIciAsync(
            ...
            if (loginResult.User.ShouldChangePasswordOnNextLogin)
            {
                ...
                return new AuthenticateResultIciModel
                {
            ...
            return new AuthenticateResultIciModel
            {
                ...
                UserId = loginResult.User.Id,
                ReturnUrl = returnUrl,
                GuestKey = loginResult.User.StaffKey,
                UserName = loginResult.User.UserName,
                logintime= DateTime.Now.ToString("MM / dd / yyyy h: mm tt")

        };

        }
        private async Task<AbpLoginResult<Tenant, User>> GetLoginResultIciAsync(string usernameOrEmailAddress,
           string password, string tenancyName)
        {
            var loginResult = await _logInManagerIcheckIn.LoginIcheckInAsync(usernameOrEmailAddress, password, tenancyName);
