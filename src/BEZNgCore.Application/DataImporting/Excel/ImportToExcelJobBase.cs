    ...
    where TInvalidEntityExporter : IExcelInvalidEntityExporter<TEntityDto>
{
    ...
    protected async Task<List<TEntityDto>> GetDataFromExcelOrNullAsync(ImportFromExcelJobArgs args)
    {
        ...
        using (CurrentUnitOfWork.SetTenantId(args.TenantId))
        {
            try
            {
                ...
                return await dataReader.GetEntitiesFromExcel(file.Bytes);
            }
            catch (Exception)
            {
                return null;
        ...
        List<TEntityDto> invalidEntities)
    {
        ...
        {
            var file = await invalidEntityExporter.ExportToFile(invalidEntities);
            await appNotifier.SomeUsersCouldntBeImported(
                args.ExcelImporter,
                file.FileToken,
                file.FileType,
                file.FileName
    ...
    private bool CanBeImported(ImportFromExcelDto row)
    {
        return string.IsNullOrEmpty(row.Exception);
    }
}