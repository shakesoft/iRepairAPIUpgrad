...
using BEZNgCore.Security;
using BEZNgCore.Timing;
using BEZNgCore.Timing.Dto;
...
public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
{
        ...
        return new HostUserManagementSettingsEditDto
        {
            ...
            AllowUsingGravatarProfilePicture =
                await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement
                    .AllowUsingGravatarProfilePicture),
            MaxProfilePictureSizeInMB =
                await SettingManager.GetSettingValueAsync<float>(AppSettings.UserManagement.MaxProfilePictureSizeInMB),
    ...
    private async Task UpdateUserManagementSettingsAsync(HostUserManagementSettingsEditDto settings)
    {
        ...
        await UpdateProfilePictureSettingsAsync(settings);
    ...
    private async Task UpdateProfilePictureSettingsAsync(HostUserManagementSettingsEditDto settings)
    {
        await SettingManager.ChangeSettingForApplicationAsync(
            AppSettings.UserManagement.MaxProfilePictureSizeInMB,
            settings.MaxProfilePictureSizeInMB.ToString()
        );

        await SettingManager.ChangeSettingForApplicationAsync(
            AppSettings.UserManagement.MaxProfilePictureWidth,
            settings.MaxProfilePictureWidth.ToString()
        );

        await SettingManager.ChangeSettingForApplicationAsync(
            AppSettings.UserManagement.MaxProfilePictureHeight,
            settings.MaxProfilePictureHeight.ToString()
        );
    }
