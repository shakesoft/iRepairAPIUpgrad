...
public class TenantAppService : BEZNgCoreAppServiceBase, ITenantAppService
{
    ...
    public async Task<PagedResultDto<TenantListDto>> GetTenants(GetTenantsInput input)
    {
        ...
        bool isUserLockOutEnabledForHost = await IsUserLockOutEnabledForHostAsync();

        if (isUserLockOutEnabledForHost)
        {
            return await GetTenantsWithLockoutStatus(tenantCount, tenants);
        }
    ...
    private async Task<PagedResultDto<TenantListDto>> GetTenantsWithLockoutStatus(int tenantCount, List<Tenant> tenants)
    {
        var tenantDtos = new List<TenantListDto>();

        foreach (var tenant in tenants)
        {
            var tenantDto = ObjectMapper.Map<TenantListDto>(tenant);
            tenantDto.IsTenantAdminLocked = await IsTenantAdminLocked(new EntityDto { Id = tenant.Id });
    ...
    private async Task<bool> IsTenantAdminLocked(EntityDto input)
    {
        using (CurrentUnitOfWork.SetTenantId(input.Id))
        {
            var tenantAdmin = await UserManager.GetAdminAsync();
            return tenantAdmin != null && await UserManager.IsLockedOutAsync(tenantAdmin);
        }
    }

    private async Task<bool> IsUserLockOutEnabledForHostAsync()
    {
        return await SettingManager.GetSettingValueForApplicationAsync<bool>(AbpZeroSettingNames.UserManagement.UserLockOut.IsEnabled);
    }
}
